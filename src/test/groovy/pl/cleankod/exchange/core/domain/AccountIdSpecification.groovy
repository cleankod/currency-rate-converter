...
class AccountIdSpecification extends Specification { // whole tests doesn't make sense anymore as UUID is parsed through UUID class itself
    def "should create object for valid value"() {
        when:
        UUID accountId = UUID.fromString(givenValue)
...
//        when:
//        Account.id(UUID.fromString(null as String))
//
//        then:
