...
class AccountSpecification extends BaseApplicationSpecification {
    ...
    def "should return an account by ID"() {
        ...
        when:
        AccountWrapper response = get("/accounts/${accountId}", AccountWrapper)

        then:
        response == new AccountWrapper(
                UUID.fromString(accountId),
                "65 1090 1665 0000 0001 0373 7343",
                new MoneyWrapper(new BigDecimal("123.45"), Currency.getInstance("PLN"))
        )
    }
    ...
    def "should return an account by ID with different currency"() {
        ...
        when:
        AccountWrapper response = get("/accounts/${accountId}?currency=${currency}", AccountWrapper)

        then:
        response == new AccountWrapper(
                UUID.fromString(accountId),
                "65 1090 1665 0000 0001 0373 7343",
                new MoneyWrapper(new BigDecimal("27.16"),Currency.getInstance(currency))
        )
    }
    ...
    def "should return an account by number"() {
        ...
        when:
        AccountWrapper response = get("/accounts/number=${accountNumberUrlEncoded}", AccountWrapper)

        then:
        response == new AccountWrapper(
                UUID.fromString("78743420-8ce9-11ec-b0d0-57b77255c208"),
                accountNumberValue,
                new MoneyWrapper(new BigDecimal("456.78"), Currency.getInstance("EUR"))
        )
    }
